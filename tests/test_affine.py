...
class TestEqualityBoxSetLowering:
    ...
    def test_spec_example_symbolic_stays_affineset(self):
        """Spec example with symbols: affine_set<(p)[c] : (p - c + 2 == 0)>.

        n_syms > 0, so try_from_affine_set rejects it; stays AffineSet.
        """
